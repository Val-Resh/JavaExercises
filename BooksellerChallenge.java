...
public class BooksellerChallenge {
    ...
    */
    ...
    public static String stockSummary (String[] listOfArt, String[] listOf1stLetter) {
        ...
        for (String x :  listOf1stLetter){
            map.put(x, 0);
        }
        if (listOf1stLetter.length == 0 || listOfArt.length == 0) return solution.toString();
        else {
            for (String x : listOf1stLetter) {
                for (String y : listOfArt) {
                    if (y.charAt(0) == x.charAt(0)) {
                        StringBuilder temporary = new StringBuilder();
                        for (int i = 0; i < y.length(); i++) {
                            if (numbers.contains((y.charAt(i)))) {
                                temporary.append(y.charAt(i));
                            }
                        }
                        int num = Integer.parseInt(temporary.toString());
                        map.replace(x, map.get(x) + num);
                    }
                }
            }
            for (String key : map.keySet()) {
                solution.append("(").append(key).append(" : ").append(map.get(key)).append(")");
            }
            solution = new StringBuilder(solution.toString().replaceAll("\\)\\(", "\\) - \\("));
        }
